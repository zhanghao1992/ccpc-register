...
//    console.log('xxx1');
    vService.request(req, res, {path: '/apply/createCaptcha'}, function (json) {
...
//            res.writeHead('200', {'Content-Type': json.response.contentType, 'Content-Disposition': 'attachment; filename=' + json.response.fileName});
            ...
            res.writeHead('200', {
                'Content-Type': json.response.contentType,
                'Content-Disposition': 'inline;filename=' + encodeURIComponent(json.response.fileName)
            });
            res.end(new Buffer(json.response.base64String, 'base64').toString('binary'), 'binary');
...
router.post('/upload', upload.single('file'), function (req, res) {
    vService.request(req, res, {
        path: '/common-file/uploadBase64', url: 'http://172.21.120.241:8080/', addParams: {
